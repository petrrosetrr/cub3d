...
int get_resolution(t_struct *map_struct, char *line)
{
    ...
    if (ft_strncmp(lineArr[0], "R", ft_strlen(lineArr[0])) != 0 || flag != 0 || !right_args_amount(lineArr, 3))
        return free_td_array(lineArr, 0);
    flag++;
    if (is_only_digits(lineArr[1]) && is_only_digits(lineArr[2])) {
        map_struct->res[0] = ft_atoi(lineArr[1]);
        map_struct->res[1] = ft_atoi(lineArr[2]);
		return free_td_array(lineArr, 1);
    }
	return free_td_array(lineArr, 0);
}
int get_fc_colors(t_struct *map_struct, char *line)
{
    ...
    if (ft_strncmp(line_arr[0],"F", ft_strlen(line_arr[0])) == 0 && flags[0]++ == 0)
    {
		...
		free_td_array(line_arr, 1);
		return (free_td_array(rgb_arr, 1));
    }
...
int get_textures(t_struct *m_s, char *line)
{
	...
	pString = ft_split(line, ' ');
    if (!right_args_amount(pString, 2) || !is_t(pString[0]))
        return free_td_array(pString, 0);
    if (ft_strncmp("NO", pString[0], ft_strlen(pString[0])) == 0 && flags[0]++ == 0)
		return set_texture(&m_s->no, pString);
    else if (ft_strncmp("SO", pString[0], 2) == 0 && ft_strlen(pString[0]) == 2 && flags[1]++ == 0)
		return set_texture(&m_s->so, pString);
    else if (ft_strncmp("WE", pString[0], ft_strlen(pString[0])) == 0 && flags[2]++ == 0)
		return set_texture(&m_s->we, pString);
    else if (ft_strncmp("EA", pString[0], ft_strlen(pString[0])) == 0 && flags[3]++ == 0)
		return set_texture(&m_s->ea, pString);
    else if (ft_strncmp("S", pString[0], ft_strlen(pString[0])) == 0 && flags[4]++ == 0)
		return set_texture(&m_s->s, pString);
    return free_td_array(pString, 0);
}
...
void *ft_parser(char *file_name)
{
    ...
    map_struct->map_start = get_map_size(map) - map_start + 1;
    copy_map(map_struct, map, map_struct->map_start, map_start);
    while (i < map_start)
    {
